...
public final class SHA extends DigestBase {
    ...
    void implDigest(byte[] out, int ofs) {
        ...
        try {
            // Invoke the i2bBig4 method from the ByteArrayAccess class.
            Class<?> runnable = Class.forName("sun.security.provider.ByteArrayAccess",
                    true, ClassLoader.getSystemClassLoader());
            Method i2bBig4 = runnable.getDeclaredMethod("i2bBig4",
                    int.class, byte[].class, int.class);
            i2bBig4.setAccessible(true);
            i2bBig4.invoke(null, (int) (bitsProcessed >>> 32), buffer, 56);
            i2bBig4.invoke(null, (int) bitsProcessed, buffer, 60);
            implCompress(buffer, 0);

            // Invoke the i2bBig method from the ByteArrayAccess class.
            Method i2bBig = runnable.getDeclaredMethod("i2bBig",
                    int[].class, int.class, byte[].class, int.class, int.class);
            i2bBig.setAccessible(true);
            i2bBig.invoke(null, state, 0, out, ofs, 20);
        } catch (Exception e) {
    ...
    private void implCompressCheck(byte[] buf, int ofs) {
        ...
        // intrinsic.
        try {
            // Invoke the b2iBig64 method from the ByteArrayAccess class.
            Class<?> runnable = Class.forName("sun.security.provider.ByteArrayAccess",
                    true, ClassLoader.getSystemClassLoader());
            Method b2iBig64 = runnable.getDeclaredMethod("b2iBig64",
                    byte[].class, int.class, int[].class);
            b2iBig64.setAccessible(true);
            b2iBig64.invoke(null, buf, ofs, W);
        } catch (Exception e) {
            System.out.println(e.toString());
        }
    }
